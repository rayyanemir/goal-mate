...
5. Cara implementasi step by step:
   ...
   Tugas 6:
1. Synchronous request adalah jenis permintaan di mana client (biasanya browser) harus menunggu hingga server selesai memproses permintaan dan mengirimkan respons sebelum dapat melakukan tindakan lain. Ini menyebabkan halaman web “membeku” sementara proses berjalan. Sebaliknya, asynchronous request memungkinkan client mengirim permintaan ke server tanpa menghentikan aktivitas lain di halaman. Browser tetap responsif karena proses pengambilan data berjalan di latar belakang, dan hasilnya baru akan ditampilkan ketika server merespons. Dengan kata lain, synchronous bersifat “blokir”, sedangkan asynchronous memungkinkan multitasking dan pengalaman pengguna yang lebih halus.

2. (Asynchronous JavaScript and XML) bekerja di Django dengan mengirimkan request ke server melalui JavaScript, biasanya menggunakan objek XMLHttpRequest atau library seperti jQuery atau Fetch API, tanpa memuat ulang seluruh halaman. Ketika AJAX dikirim, Django menerima request tersebut seperti request biasa—melalui URL yang terhubung ke view tertentu. View Django kemudian memproses data (misalnya query database) dan mengembalikan respons dalam format JSON atau HTML parsial. JavaScript di sisi client akan menerima respons itu dan memperbarui bagian tertentu dari halaman (misalnya tabel, form, atau notifikasi) tanpa perlu reload penuh, menciptakan interaksi yang cepat dan dinamis.

3. Menggunakan AJAX memberikan efisiensi dan pengalaman pengguna yang lebih baik karena tidak memerlukan reload seluruh halaman setiap kali ada interaksi dengan server. Hanya bagian tertentu dari halaman yang diperbarui, sehingga waktu respons menjadi lebih cepat dan penggunaan bandwidth lebih hemat. Selain itu, AJAX memungkinkan pembuatan fitur interaktif seperti live search, form validation real-time, infinite scrolling, dan notifikasi dinamis tanpa gangguan tampilan. Dibandingkan render biasa yang cenderung statis dan memerlukan refresh halaman penuh, AJAX menciptakan alur kerja yang lebih halus dan modern.

4.Untuk menjaga keamanan fitur Login dan Register yang menggunakan AJAX, beberapa langkah penting perlu diterapkan. Pertama, selalu gunakan CSRF token agar Django dapat memverifikasi bahwa permintaan berasal dari pengguna yang sah, bukan dari serangan Cross-Site Request Forgery. Kedua, gunakan HTTPS untuk mengenkripsi data sensitif seperti password agar tidak mudah disadap. Ketiga, lakukan validasi data di sisi server (bukan hanya di JavaScript) untuk mencegah manipulasi data atau bypass validasi. Selain itu, batasi jumlah request dan gunakan reCAPTCHA untuk menghindari serangan brute-force. Dengan langkah-langkah ini, AJAX tetap aman digunakan untuk autentikasi.

5.AJAX sangat meningkatkan pengalaman pengguna karena memungkinkan interaksi yang cepat, responsif, dan tanpa gangguan reload halaman. Pengguna dapat melihat perubahan data secara langsung, seperti hasil pencarian otomatis atau pembaruan konten real-time, yang membuat website terasa lebih modern dan interaktif. Waktu tunggu yang lebih singkat juga mengurangi frustrasi pengguna dan meningkatkan kepuasan secara keseluruhan. Selain itu, AJAX membuat navigasi terasa lebih alami karena halaman tidak terus berkedip akibat reload, sehingga pengalaman menjadi lebih mulus dan efisien.
